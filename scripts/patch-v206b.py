...
'''
if 'const RUN_ITEMS=[' not in s:
    s=s.replace('const CHALLENGES=[', insert+'const CHALLENGES=[', 1)
...
new=r'''function relicMods(run){const out={};for(const id of run.relics||[]){const x=RELICS.find(r=>r.id===id);for(const [k,v] of Object.entries(x?.mods||{}))out[k]=(out[k]||0)+v}return out}
...
s=s.replace(needle,'startedAt:Date.now(),formation:0,items:{},curses:[],nextBattleMods:{}}',1)
...
needle="run.log.push(`宝箱：+${base} 灵能，并获得一份远征材料。`);if(Math.random()<.45)offerRelic(run);else advance(run)"
if needle not in s:
    raise SystemExit('treasure target not found')
s=s.replace(needle,"run.log.push(`宝箱：+${base} 灵能，并获得一份远征材料。`);if(Math.random()<.55){const it=grantRunItem(run,rand(RUN_ITEMS).id);run.log.push(`发现远征道具：${it.name} ×1。`)}if(Math.random()<.45)offerRelic(run);else advance(run)",1)

needle="if(Math.random()<.35)offerRelic(run);else advance(run)}else{run.supply=Math.max(0,run.supply-1);run.hp[m.id]=Math.max(1,hpPct(run,m.id)-18);run.log.push(`事件失败：${monsterName(m)} 受伤 18%，补给 -1。`);advance(run)}}"
if needle not in s:
    raise SystemExit('challenge target not found')
s=s.replace(needle,"if(Math.random()<.30){const it=grantRunItem(run,rand(RUN_ITEMS).id);run.log.push(`额外发现：${it.name} ×1。`)}if(Math.random()<.35)offerRelic(run);else advance(run)}else{run.supply=Math.max(0,run.supply-1);run.hp[m.id]=Math.max(1,hpPct(run,m.id)-18);run.log.push(`事件失败：${monsterName(m)} 受伤 18%，补给 -1。`);if(Math.random()<.55){const pool=CURSES.filter(c=>!run.curses?.includes(c.id));if(pool.length)inflictCurse(run,rand(pool).id,run.log)}advance(run)}}",1)
...
needle="if(ev.target.closest?.('[data-rg-start]')){startRun();return}"
if needle not in s:
    raise SystemExit('click handler target not found')
s=s.replace(needle,needle+"const item=ev.target.closest?.('[data-rg-item]');if(item){useRunItem(item.dataset.rgItem);return}",1)
