...
function prepareEggMilestone(run){const f=floorNo(run.stage),d=Number(run.difficulty)||0;run.eggMilestoneChoices=[
...
function runHeader(run){const zone=z(run.zone),d=diff(run.difficulty),chapter=Math.floor(run.stage/9)+1;return `<section class="rg-panel"><div class="rg-title"><div><b>${zone.label} · ${d.label}</b><small>当前楼层 ${floorLabel(run.stage)} · 每个 X-9 为楼层BOSS${floorNo(run.stage)>3?' · 无限加成 +'+Math.round(endlessExtra(run.stage)*100)+'%':''}</small></div><button class="secondary" data-rg-abandon>退出（失败·25%）</button></div>${routeBar(run)}<div class="rg-status"><span>补给 ${run.supply}</span><span>积分 ${Math.round(run.score||0)}</span><span>遗物 ${(run.relics||[]).length} · 种类 ${Object.keys(relicCounts(run)).length}</span></div><div class="rg-earned"><b>目前已获得奖励</b><span>灵能 ${Math.round(run.energy)}</span><span>临时徽章 ${run.tempBadges}</span><span>遗物尘 ${run.materials?.relicDust||0}</span><span>星辉结晶 ${run.materials?.starCrystal||0}</span><span>基础蛋碎片 ${run.materials?.eggFragment||0}</span><span>远征秘藏碎片 ${run.materials?.expeditionEggFragment||0}</span><span>闪光蛋碎片 ${run.materials?.shinyEggFragment||0}</span></div></section>`}
...
window.QinsterExpedition={render,zones:ZONES,version:'v239-score-meta'};
