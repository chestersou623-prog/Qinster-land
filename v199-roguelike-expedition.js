...
function expeditionStats(m){const base=ranchStats(m),mul=speciesBattleMultipliers(m),trait=expeditionTraitFor(m),shiny=m?.shiny?1.05:1;return base.map((v,i)=>Math.round((((Number(v)||0)*(mul[i]||1)+(Number(trait.stat)===i?Number(trait.flat)||0:0))*shiny)*10)/10)}
...
function applyExpeditionLifeCost(run,cleared,defeated=false){
  ...
  const byId=new Map([...(s.monsters||[]),...(s.expedition?.box||[])].map(m=>[m.id,m])),notes=[],deadIds=[];
  if(cleared){
    for(const id of run.teamIds||[]){
  ...
  if(deadIds.length){
    ...
    if(Array.isArray(s.expedition?.box))s.expedition.box=s.expedition.box.filter(m=>!dead.has(Number(m.id)));s.monsters=(s.monsters||[]).filter(m=>!dead.has(Number(m.id)));
...
window.QinsterExpedition={render,zones:ZONES,version:'v269'};
