...
function enemyPreview(run,kind){const zone=z(run.zone),d=diff(run.difficulty),elite=kind==='elite',boss=kind==='boss',local=run.stage%9,chapter=Math.floor(run.stage/9),base=zone.enemy*(1+local*.05+chapter*.18)*(elite?1.18:1)*(boss?1.32:1),range=boss?[1.20,1.45]:elite?[1.05,1.25]:[.90,1.10],variance=range[0]+Math.random()*(range[1]-range[0]),mods=d.mods||{},speciesCount=Math.max(1,R()?.G?.SPECIES?.length||1),enemySpecies=Math.floor(Math.random()*speciesCount),affixPool=[{name:'狂暴',mods:{atk:.20}},{name:'铁壁',mods:{def:.25}},{name:'迅捷',mods:{spd:.20}},{name:'强运',mods:{luck:.25}},{name:'巨躯',mods:{hp:.25}},{name:'精准',mods:{atk:.08,luck:.15}}],affixCount=d.id>=10?3:d.id>=7?2:d.id>=4?1:0,affixes=shuffle(affixPool).slice(0,affixCount),am={};for(const a of affixes)for(const [k,v] of Object.entries(a.mods))am[k]=(am[k]||0)+v;const mult=base*variance,hpMul=1+(mods.hp||0)+(am.hp||0),atkMul=1+(mods.atk||0)+(am.atk||0),defMul=1+(mods.def||0)+(am.def||0),spdMul=1+(mods.spd||0)+(am.spd||0),luckMul=1+(mods.luck||0)+(am.luck||0);return{kind,enemySpecies,enemyShiny:zone.shinyOnly,enemyMax:Math.round((520*mult+100)*hpMul),enemyAtk:Math.round((42*mult+8)*atkMul),enemyDef:Math.round((35*mult+7)*defMul),enemySpd:Math.round((30*mult+6)*spdMul),enemyLuck:Math.round((26*mult+5)*luckMul),affixes:affixes.map(x=>x.name),variance:Math.round(variance*100)}}
...
window.QinsterExpedition={render,zones:ZONES,version:'v223-route-erosion'};
