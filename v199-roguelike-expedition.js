...
function battle(run,kind){
  ...
  while(livingEnemies().length&&actions<maxActions&&t.some(m=>hpPct(run,m.id)>0&&canReviveInRun(run,m.id))){
    ...
    if(ready.type==='ally'){
      ...
      if(mode==='aoe'){
        const targets=current.map(x=>hurtAlly(e,x,.64));events.push({type:'enemy',action:actions,enemyId:e.id,enemyIndex:enemies.indexOf(e),aoe:true,skillName:'全体攻击',attackType:'全体攻击',targets,spd:ready.spd,wait:dt,gauges:snap});logs.push(`行动 ${actions}：${e.name} 使用【全体攻击】，攻击全队。`);
      }else{
        const roll=Math.random(),target=roll<.64?current[0]:roll<.88?(current[1]||current[0]):(current[2]||current[1]||current[0]),res=hurtAlly(e,target,mode==='skill'?1.45:1);events.push({type:'enemy',action:actions,enemyId:e.id,enemyIndex:enemies.indexOf(e),targetId:res.targetId,hpLoss:res.hpLoss,hpAfter:res.hpAfter,miss:res.miss,skillName:mode==='skill'?'强袭技能':'',attackType:mode==='skill'?'强袭技能':'普通攻击',spd:ready.spd,wait:dt,gauges:snap});logs.push(`行动 ${actions}：${e.name} 使用【${mode==='skill'?'强袭技能':'普通攻击'}】攻击 ${monsterName(target)}${res.miss?'，但被闪避。':`，远征生命 -${Math.round(res.hpLoss)}%`}。`)
      }
      gauge[e.id]=0;
    }
    tickStatus();
...
window.QinsterExpedition={render,zones:ZONES,version:'v244-combat-clarity'};
