...
function battle(run,kind){
  ...
  const battleMods=combatMods(run),logs=[],events=[],gauge={},personalActions={},enemyActions={},allyBuff={atk:0,def:0,spd:0,luck:0,ttl:0},enemyDebuff={atk:0,def:0,spd:0,luck:0,ttl:0},digitSeed={},blocked={},shields={},vulnStack={};t.forEach((m,i)=>{digitSeed[m.id]=Math.random();gauge[m.id]=0;shields[m.id]=Math.max(0,Number(battleMods.shieldFlat)||0)+(i===0?Math.max(0,Number(battleMods.frontShieldFlat)||0):0)});enemies.forEach(e=>{gauge[e.id]=0;vulnStack[e.id]=0});
