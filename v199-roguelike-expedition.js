...
function ensure(){const s=S();if(!s)return null;if(!s.expedition||typeof s.expedition!=='object')s.expedition={};const e=s.expedition;if(e.dayKey!==today()){e.dayKey=today();e.usedByZone={}}if(!e.usedByZone)e.usedByZone={};if(!e.extraAttempts)e.extraAttempts={};if(!e.difficultyUnlocked)e.difficultyUnlocked={normal:0,shiny:0};for(const x of ZONES)e.difficultyUnlocked[x.id]=Math.max(0,Math.min(10,Number(e.difficultyUnlocked[x.id])||0));e.attemptPotions=Math.max(0,Number(e.attemptPotions)||0);e.badges=Math.max(0,Number(e.badges)||0);if(!e.loot)e.loot={relicDust:0,starCrystal:0,eggFragment:0};for(const k of ['relicDust','starCrystal','eggFragment','expeditionEggFragment','shinyEggFragment'])e.loot[k]=Math.max(0,Number(e.loot[k])||0);if(!e.metaTree)e.metaTree={atk:0,def:0,spd:0,luck:0,supply:0};if(!Array.isArray(e.leaderboard))e.leaderboard=[];if(!selected.length)selected=(e.lastTeamIds||[]).slice(0,3);if(!zoneInitialized){selectedZone=ZONES.some(x=>x.id===e.lastZone)?e.lastZone:'normal';selectedDifficulty=Math.min(Number(e.lastDifficulty)||0,e.difficultyUnlocked[selectedZone]||0);zoneInitialized=true}return e}
...
function startRun(){const e=ensure(),zone=z(),t=team(),d=diff();if(!e)return;if(selectedDifficulty>(e.difficultyUnlocked?.[zone.id]||0))return R()?.tell?.('这个难度还没有解锁。');if(t.length!==3)return R()?.tell?.('需要选择 3 只怪物。');if(new Set(t.map(m=>m.id)).size!==3)return R()?.tell?.('不能重复选择同一只怪物。');if(zone.shinyOnly&&t.some(m=>!m.shiny))return R()?.tell?.('闪光远征只允许闪光怪物参加。');if(totalRemain(e,zone)<=0)return R()?.tell?.('这个地图与难度今天的免费次数已用完；可使用远征次数回复药水继续。');const k=attemptKey(zone);if(freeRemain(e,zone)>0)e.usedByZone[k]=used(e,zone)+1;else e.extraAttempts[k]=Math.max(0,bonusAttempts(e,zone)-1);e.lastZone=zone.id;e.lastDifficulty=selectedDifficulty;e.lastTeamIds=t.map(m=>m.id);const hp={};t.forEach(m=>hp[m.id]=100);e.rogueActive={zone:zone.id,difficulty:selectedDifficulty,teamIds:t.map(m=>m.id),stage:0,maxStage:27,hp,supply:5+(metaMods(e).supply||0),relics:[],energy:0,tempBadges:0,materials:{relicDust:0,starCrystal:0,eggFragment:0},options:makeOptions(0),phase:'map',log:[`${zone.label} · ${d.label} 开始。目标：击败第9/18/27层三名大BOSS。`],startedAt:Date.now(),formation:0,items:{},curses:[],nextBattleMods:{},bossCount:0,score:0,kills:{normal:0,elite:0,boss:0}};save(`${zone.label} · ${d.label} 开始：共27层，每9层一个大BOSS。`)}
...
function resolveChallenge(run,memberIndex){const t=activeTeam(run),m=t[memberIndex],c=run.challenge;if(!m||!c)return;const zone=z(run.zone),mods=relicMods(run),v=st(m)[c.stat],bonus=c.stat===4?(mods.luck||0)*v:c.stat===3?(mods.spd||0)*v:c.stat===2?(mods.def||0)*v:c.stat===1?(mods.atk||0)*v:0,target=70+zone.tier*58+run.stage*12,p=Math.max(.18,Math.min(.92,.52+(v+bonus-target)/Math.max(100,target)*.62));const success=Math.random()<p;let lines=[],after='advance';if(success){const gain=Math.round((250+run.stage*75)*nodeRewardScale(zone,run.stage));run.energy+=gain;lines.push(`事件成功：${monsterName(m)} 用${STAT[c.stat]}通过判定，+${gain} 灵能。`);if(Math.random()<.30){const it=grantRunItem(run,rand(RUN_ITEMS).id);lines.push(`额外发现：${it.name} ×1。`)}if(Math.random()<.35)after='relic'}else{run.supply=Math.max(0,run.supply-1);run.hp[m.id]=Math.max(1,hpPct(run,m.id)-18);lines.push(`事件失败：${monsterName(m)} 受伤 18%，补给 -1。`);if(Math.random()<.55){const pool=CURSES.filter(x=>!run.curses?.includes(x.id));if(pool.length){const before=run.log.length;inflictCurse(run,rand(pool).id,lines)}}}run.log.push(...lines);run.challengeResult={success,memberId:m.id,memberName:monsterName(m),stat:STAT[c.stat],value:Math.round(v+bonus),baseValue:v,target,chance:Math.round(p*100),title:c.title,lines,after};run.phase='challengeResult';save(success?'特殊事件成功！':'特殊事件失败。')}
...
function prepareEggMilestone(run){const f=floorNo(run.stage),d=Number(run.difficulty)||0,rewardMul=1+floorRewardBonus(run.stage),scale=v=>Math.max(1,Math.round(v*rewardMul));run.eggMilestoneChoices=[
...
function eggMilestoneHTML(run){const cs=run.eggMilestoneChoices||[];return `${runHeader(run)}${runInventoryHTML(run)}<section class="rg-panel"><div class="rg-title"><div><b>🥚 每3楼层里程碑</b><small>击败第3个楼层BOSS后，从三种蛋碎片中选择一种</small></div><span>${floorLabel(run.stage)}</span></div><div class="rg-final">${cs.map((c,i)=>`<button class="primary rg-relic" data-rg-egg-milestone="${i}"><b>${c.name} ×${c.value}</b><span>${c.text}</span></button>`).join('')}</div></section>`}
function floorBossChoiceHTML(run){const next=floorNo(run.stage)+1,enemyExtra=Math.max(0,next-3)*10,rewardExtra=Math.max(0,next-3)*5;return `${runHeader(run)}${runInventoryHTML(run)}<section class="rg-panel rg-floor-choice"><div class="rg-title"><div><b>${floorLabel(run.stage)} 楼层BOSS已击败</b><small>现在是安全结算点。继续后进入 ${next}-1；敌方全能力与局内奖励使用独立倍率。</small></div></div><div class="rg-final"><button class="primary rg-relic" data-rg-floor-cashout><b>领取全部奖励并结束</b><span>以 100% 结算目前已获得奖励</span></button><button class="secondary rg-relic" data-rg-floor-continue><b>继续无限模式</b><span>进入 ${next}-1 · 敌方所有能力 +${enemyExtra}% · 本层局内奖励 +${rewardExtra}%</span></button></div></section>`}
...
window.QinsterExpedition={render,zones:ZONES,version:'v242-multi-enemy'};
