...
function runHeader(run){const zone=z(run.zone),d=diff(run.difficulty),chapter=Math.floor(run.stage/9)+1;return `<section class="rg-panel"><div class="rg-title"><div><b>${zone.label} · ${d.label}</b><small>当前楼层 ${floorLabel(run.stage)} · 每个 X-9 为楼层BOSS${floorNo(run.stage)>3?' · 无限加成 +'+Math.round(endlessExtra(run.stage)*100)+'%':''}</small></div><button class="secondary" data-rg-abandon>退出（失败·25%）</button></div>${routeBar(run)}<div class="rg-status"><span>补给 ${run.supply}</span><span>遗物 ${(run.relics||[]).length} · 种类 ${Object.keys(relicCounts(run)).length}</span></div><div class="rg-earned"><b>目前已获得奖励</b><span>灵能 ${Math.round(run.energy)}</span><span>临时徽章 ${run.tempBadges}</span><span>遗物尘 ${run.materials?.relicDust||0}</span><span>星辉结晶 ${run.materials?.starCrystal||0}</span><span>蛋碎片 ${run.materials?.eggFragment||0}</span></div></section>`}
...
function activeHTML(run){if(run.phase==='floorBossChoice')return floorBossChoiceHTML(run);if(run.phase==='training')return trainingHTML(run);if(run.phase==='temple')return templeHTML(run);if(run.phase==='camp')return campHTML(run);if(run.phase==='templeResult')return templeResultHTML(run);if(run.phase==='battlePreview')return battlePreviewHTML(run);if(run.phase==='battleResult')return battleHTML(run);if(run.phase==='challenge')return challengeHTML(run);if(run.phase==='challengeResult')return challengeResultHTML(run);if(run.phase==='relic')return relicHTML(run);if(run.phase==='final')return finalHTML(run);return `${runHeader(run)}${runInventoryHTML(run)}${teamHTML(run)}${nodeHTML(run)}<details class="rg-panel"><summary>本局遗物与记录</summary>${relicTrayHTML(run)}<div class="rg-log">${run.log.slice(-8).reverse().map(x=>`<div>${x}</div>`).join('')}</div></details>`}
...
window.QinsterExpedition={render,zones:ZONES,version:'v236-endless-core'};
