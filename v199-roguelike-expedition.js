...
function ensureRunMeta(run){if(!run.items)run.items={};if(!run.curses)run.curses=[];if(!run.nextBattleMods)run.nextBattleMods={};if(!run.templeMods)run.templeMods={};if(!run.trainingMods)run.trainingMods={};return run}
...
function activeHTML(run){if(run.phase==='training')return trainingHTML(run);if(run.phase==='camp')return campHTML(run);if(run.phase==='templeResult')return templeResultHTML(run);if(run.phase==='battlePreview')return battlePreviewHTML(run);if(run.phase==='battleResult')return battleHTML(run);if(run.phase==='challenge')return challengeHTML(run);if(run.phase==='challengeResult')return challengeResultHTML(run);if(run.phase==='relic')return relicHTML(run);if(run.phase==='final')return finalHTML(run);return `${runHeader(run)}${runInventoryHTML(run)}${teamHTML(run)}${nodeHTML(run)}<details class="rg-panel"><summary>本局遗物与记录</summary><p>${(run.relics||[]).map(id=>RELICS.find(x=>x.id===id)?.name).filter(Boolean).join(' · ')||'暂时没有遗物'}</p><div class="rg-log">${run.log.slice(-8).reverse().map(x=>`<div>${x}</div>`).join('')}</div></details>`}
